...
for subj in subjects:
    ...
    if os.path.exists(data_dir+subj+'/act.csv'):
        data_act = pd.read_csv(data_dir+subj+'/act.csv',sep='\t',header=None)
        # convert timestamps to daystamps
        data_act[0] = np.floor((data_act[0]+ 3600*float(timezones.loc[timezones[0]==subj,1]))/86400.0)
        # loading into new matrix
        act['date'] = np.arange(data_act.loc[0,0],data_act.loc[data_act.shape[0]-1,0])
        for (i,da) in enumerate(act['date']):
             act.loc[i,'act'] = np.sum(data_act.loc[data_act[0]==da,1]=='BIKING')+np.sum(data_act.loc[data_act[0]==da,1]=='ON_FOOT')
    else:
        print ' no act data'
    acts.append(act)
