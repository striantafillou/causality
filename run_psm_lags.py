...
for k in range(n_boot):
    ...
    for (c,i) in enumerate(inds):
        ...
        treatment = np.array(data[i]['mood_prev'])
        outcome = np.array(data[i]['quality'])
        confound = np.array(pd.concat([data[i]['act_prev'],data[i]['stress_prev'],data[i]['energy_prev'],data[i]['daytype'],                                      data[i]['quality_prev']],axis=1))
        es_m2s[c,1] = psm_causal_effects(treatment=treatment, outcome=outcome, confound=confound, scorefun='replacement')

        # mood on sleep - lag 2
        treatment = np.array(data[i]['mood_prev'])
        outcome = np.array(data[i]['quality'])
        confound = np.array(pd.concat([data[i]['act_prev'],data[i]['stress_prev'],data[i]['energy_prev'],data[i]['daytype'],                                      data[i]['quality_prev'],data[i]['quality_prev2']],axis=1))
        es_m2s[c,2] = psm_causal_effects(treatment=treatment, outcome=outcome, confound=confound, scorefun='replacement')

        # mood on sleep - lag 3
        treatment = np.array(data[i]['mood_prev'])
        outcome = np.array(data[i]['quality'])
        confound = np.array(pd.concat([data[i]['act_prev'],data[i]['stress_prev'],data[i]['energy_prev'],data[i]['daytype'],                                      data[i]['quality_prev'],data[i]['quality_prev2'],data[i]['quality_prev3']],axis=1))
        ...
        treatment = np.array(data[i]['quality'])
        outcome = np.array(data[i]['mood'])
        confound = np.array(pd.concat([data[i]['stress_prev'],data[i]['energy_prev'],data[i]['daytype'],                                      data[i]['mood_prev']],axis=1))
        es_s2m[c,1] = psm_causal_effects(treatment=treatment, outcome=outcome, confound=confound, scorefun='replacement')
        
        # sleep on mood - lag 2
        treatment = np.array(data[i]['quality'])
        outcome = np.array(data[i]['mood'])
        confound = np.array(pd.concat([data[i]['stress_prev'],data[i]['energy_prev'],data[i]['daytype'],                                      data[i]['mood_prev'],data[i]['mood_prev2']],axis=1))
        es_s2m[c,2] = psm_causal_effects(treatment=treatment, outcome=outcome, confound=confound, scorefun='replacement')
        
        # sleep on mood - lag 3
        treatment = np.array(data[i]['quality'])
        outcome = np.array(data[i]['mood'])
        confound = np.array(pd.concat([data[i]['stress_prev'],data[i]['energy_prev'],data[i]['daytype'],                                      data[i]['mood_prev'],data[i]['mood_prev2'],data[i]['mood_prev3']],axis=1))
